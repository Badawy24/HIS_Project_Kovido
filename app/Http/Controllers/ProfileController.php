...
class ProfileController extends Controller
{
    ...
    public function updateprofile(Request $request)
    {
        ...
        $pat_id = session('user_id');
        DB::update('update patient set pat_email = ?, pat_address =? , pat_phone =? , pat_DOF = ? where pat_id = ?', [$request->pat_email, $request->pat_address, $request->pat_phone, $request->birthday, $pat_id]);
        $patient = DB::select('select * from patient where pat_id = ?', [$pat_id]);
